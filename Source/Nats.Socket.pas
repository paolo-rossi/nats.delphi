...
type
  INatsSocket = interface
    ...
    procedure Open();
    procedure Close();
    procedure SendBytes(const AValue: TBytes); // Sends AValue + CRLF
    procedure SendString(const AValue: string); // Sends AValue + CRLF
    function ReceiveString: string;
    // Receives a line ending with CRLF, returns line without CRLF
    function ReceiveBytes: TBytes;
    // Receives a line ending with CRLF, returns line as bytes without CRLF
    function ReceiveExactBytes(ACount: Integer): TBytes;
    // New: Receives exactly ACount bytes
    // Properties
  ...
  public
    ...
    function ReceiveExactBytes(ACount: Integer): TBytes; virtual; abstract;
    // New
  end;
  ...
  public
    ...
    class procedure Register<T: TNatsSocket>(const AName: string;
      ADefault: Boolean); static;
  end;
...
begin
  if Self.Count < 1 then
    raise ENatsException.Create
      ('CreateSocket: no socket class registered (add "Nats.Socket.*" unit to the project)');
...
begin
  ...
  begin
    ...
    if AtomicCmpExchange(Pointer(FInstance), Pointer(LRegistry), nil) <> nil
    then
      LRegistry.Free
  end;
  Result := FInstance;
...
begin
  ...
  if LSelf.Count < 1 then
    raise ENatsException.Create
      ('Get: no socket class registered (add "Nats.Socket.*" unit to the project)');

  if AName = '' then
    LClass := LSelf.GetDefaultClass()
  else if not LSelf.TryGetValue(AName, LClass) then
    raise ENatsException.CreateFmt
      ('Get: socket class [%s] not registered (add "Nats.Socket.*" unit to the project)',
      [AName]);

  Result := LClass.Create;
end;

class procedure TNatsSocketRegistry.Register<T>(const AName: string;
  ADefault: Boolean);
var
  LSelf: TNatsSocketRegistry;
begin
